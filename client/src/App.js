...
function App() {
  ...
  return (
    ...
    .slice(0, 5)
    .map(trip => <Connection key={Math.random()} apiUrl={apiURl} fromId={trip.from.opuic} toId={trip.to.opuic} depTime={trip.from.time_str} arrTime={trip.to.time_str} trainNum={trip.train} trainName={trip.train_name} from={from} to={to} date={date} time={time} />)}
